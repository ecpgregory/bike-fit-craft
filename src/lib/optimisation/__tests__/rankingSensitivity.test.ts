...
describe("D-11C-1 — Sprint 11D geometric combiner, measured at fleet level", () => {
  ...
  it("the fleet leader is now the best-fitting bike, not the exact-width one", () => {
    ...
    // Sprint 12C.4: the Tarmac SL8 size 58 joined the fleet and is better
    // positioned at this target than the previous leader (size 56). The guard
    // is the property below — position beats an exact width match — not the
    // identity.
    expect(first.bikeId).toBe("specialized-tarmac-sl8-2025-58");
